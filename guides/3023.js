...
function skilld_event(skillid, handlers, event, ent, dispatch){
	...
	if([30231000, 1000].includes(skillid)){
		debuff = 1;
		clearTimeout(timer1);
		clearTimeout(timer2);
		timer1 = setTimeout(()=> {
	...
	if([30231001, 1001].includes(skillid)){
		debuff = 2;
		clearTimeout(timer2);
		clearTimeout(timer1);
		timer2 = setTimeout(()=> {
...
module.exports = {
	...
	"s-3023-2000-175-0": [{ "type": "text", "sub_type": "message", "message": 'Incoming Stun', "message_RU": "Рёв (Эвейд)!!!" },
		{ "type": "text", "sub_type": "message", "delay": 1250, "message": 'Dodge!', "message_RU": "Уклоняйся" }],
	"s-3023-2000-178-0": [{ "type": "text", "sub_type": "message", "message": 'Scratching (bleed)', "message_RU": "Крутилка (Кровоток)" }],
	"s-3023-2000-181-0": [{ "type": "text", "sub_type": "message", "message": 'Rock Wave', "message_RU": "Полоса вперед" },
		{ "type": "func", "func": SpawnVector.bind(null, 553, 90, 80, 10, 1000, 0, 4000) },
		{ "type": "func", "func": SpawnVector.bind(null, 553, 270, 80, 350, 1000, 0, 4000) }],
